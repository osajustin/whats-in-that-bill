...
async function getHouseBills(searchParams: {
  q?: string;
  status?: string;
  page?: string;
...
async function HouseBillsContent({
  searchParams,
}: {
  searchParams: { q?: string; status?: string; page?: string };
}) {
  const data = await getHouseBills(searchParams);
...
export default async function HouseBillsPage({ searchParams }: HouseBillsPageProps) {
      ...
      <section className="border-b-[3px] border-foreground">
        <div className="p-6 md:p-10">
          ...
          <h1 className="font-serif text-4xl md:text-6xl font-black text-center mb-4 tracking-tight text-balance">
            {params.q
              ? `House Results for "${params.q}"`
              : params.status
                ? `${params.status} House Bills`
                : "House Bills"}
          </h1>

          <p className="font-serif text-lg md:text-xl italic text-center text-muted-foreground max-w-2xl mx-auto">
            Legislation from the people&apos;s chamber — where representation meets population.
            Bills prefixed with H.R. originate in the House.
          </p>
        </div>
      </section>
      ...
      <section className="border-b-[3px] border-foreground">
        <div className="p-6 md:p-10">
          <div className="flex items-center gap-4 mb-6">
            <div className="h-px flex-1 bg-foreground/30" />
            <span className="font-mono text-xs tracking-[0.3em] uppercase text-muted-foreground">
              {params.q ? "Search Results" : "Recent House Legislation"}
            </span>
            <div className="h-px flex-1 bg-foreground/30" />
          </div>

          <Suspense fallback={<BillList bills={[]} isLoading />}>
            <HouseBillsContent searchParams={params} />
          </Suspense>
        </div>
      </section>
